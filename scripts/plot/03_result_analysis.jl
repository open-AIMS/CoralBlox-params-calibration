...
# from the package, so the type is shared with the Plotting submodule without
# duplication.

using CoralBloxCalib
import CoralBloxCalib.Plotting
...
f_obs_loc_map = Plotting.plot_observation_locs(CALIBRATION_STORE, VALIDATION_STORE)
save(joinpath(OUT_DIR, "obs_loc_map.png"), f_obs_loc_map)

# Functional group cover proportions
f_taxa_cover = Plotting.taxa_cover_proportions(rs_raw.raw)
save(joinpath(OUT_DIR, "locs_taxa_cov.png"), f_taxa_cover)

f_taxa_pop = Plotting.taxa_population_proportions(rs_raw.raw)
save(joinpath(OUT_DIR, "locs_taxa_pop.png"), f_taxa_pop)

f_size_class = Plotting.temporal_size_class_proportions(rs_raw.raw; fig_size=(900, 600))
save(joinpath(OUT_DIR, "locs_size.png"), f_size_class)

# Summary stats
n_validation_locs = length(VALIDATION_STORE.ltmp_unique_ids)
validation_sccs = [
    Plotting.collect_error_stats(rs_raw.raw, id, dom; observations=VALIDATION_STORE).srcc
...
stats_calib = Plotting.collect_error_stats(rs_raw.raw, dom; observations=CALIBRATION_STORE)
stats_valid = Plotting.collect_error_stats(rs_raw.raw, dom; observations=VALIDATION_STORE)
...
Plotting.save_regional_analysis_plots(
...
Plotting.save_metric_analysis_plots(
    rs_raw.raw, dom, CALIBRATION_STORE, VALIDATION_STORE, OUT_DIR
)
...
Plotting.save_location_timeseries_plots(
    rs_raw.raw, dom, CALIBRATION_STORE, VALIDATION_STORE, OUT_DIR,
