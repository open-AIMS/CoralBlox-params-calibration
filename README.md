...
Two required inputs are not bundled and must be obtained separately (see
[docs/data_sources.md](docs/data_sources.md)):

- `canonical_path` — the canonical GBR reef geopackage, generated by
  [gbrrestoration/canonical-reefs](https://github.com/gbrrestoration/canonical-reefs)
  (a pre-built `.gpkg` is available from that repository).
- `rme_domain` — a ReefMod Engine data package, available on request or via the RRAP
  M&DS Data Store.
