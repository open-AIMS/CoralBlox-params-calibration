...
# Since data has been aggregated between a few locaotions Reef IDs, GRMBPA IDs and
...
"""
    domain_idx_to_compisition(loc_data_store::LocationDataStore, domain_idx::Int64)::Int64

Given the index of a location in the domain geopackage, return the index of the same
location in the coral composition yaxarray. The majority of domain locations do not have
a coral composition data.
"""
function domain_idx_to_compisition(loc_data_store::LocationDataStore, domain_idx::Int64)::Int64
    return loc_data_store.domain_to_composition[domain_idx]
end

"""
    enough_data(df_row::DataFrameRow)
...
"""
function enough_data(df_row::DataFrameRow)
    raw_row = collect(df_row)
    n_obs = count((!).(ismissing(df)))
    if n_obs < 4
        return false
    end
