...
for i in 1:length(LIMITED_LOCATIONS)
    ...
    save(joinpath(save_dir, "taxa_props.png"), f)

    f = plot_bleaching_mortality(location_unique_id, rs_raw.raw[:, :, domain_loc_pos], rs_raw.bleaching_mortality[:, :, :, domain_loc_pos])
    save(joinpath(save_dir, "bleaching_mortality.png"), f)

    f = plot_cyclone_mortality(location_unique_id, rs_raw.raw[:, :, domain_loc_pos], dom.cyclone_mortality_scens[:, domain_loc_pos, :, scen[1, :cyclone_mortality_scenario]].data)
    save(joinpath(save_dir, "cyclone_mortality.png"), f)

    f = plot_target_proportions(location_unique_id, rm_ltmp_taxa[:, :, limited_loc_pos])
    save(joinpath(save_dir, "target_composition.png"), f)
    ...
    #save(joinpath(save_dir, "loc_map.png"), f)
    ...
    save(joinpath(save_dir, "size_class_proportions.png"), f)
end
